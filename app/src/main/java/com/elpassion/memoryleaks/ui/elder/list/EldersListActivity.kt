...
class EldersListActivity : BaseActivity(), PingView, EldersListView {
    ...
    val pingApiCall = { elderId: String, visitorId: String ->
        getPingApiCall().invoke(elderId, visitorId).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
    }
    ...
    private val onElderClickListener: (String) -> Unit = {
        PingController(pingApiCall, this).onSendPingClicked(it, getVisitorId())
